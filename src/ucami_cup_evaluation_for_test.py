...
#Read training files
dataset = 'Training'
train_days = ['2017-10-31', '2017-11-02', '2017-11-03', '2017-11-08','2017-11-10','2017-11-20']
times = ['C'] #Use 'A' for morning, 'B' for afternoon and 'C' for evening
...
labels_test_eval = pd.DataFrame(columns=['DATE_ID', 'LABEL'])
proximity_test = pd.DataFrame(columns=['DATE_ID', 'OBJECT'])

dataset = 'Training'
test_days = ['2017-11-15']
...
for day in test_days:
    for time in times:
        ...
        labels_test = pd.read_csv('../data/string-labels-'+day+'-'+time+'-testactivity.csv')
        labels_test_eval = pd.DataFrame(columns=['DATE_ID', 'LABEL'])
        labels_test_eval = af.read_labels(day, time)
        #Complete label data
        labels_test_eval['DATE_ID']=labels_test_eval.apply(lambda x:i*10000+x['TIME_ID'], axis=1)
...
Y_test, classes_t = p.get_Y(labels_test_eval)
...
for t in X_test.iterrows():
    ...
    '''
    prediction = prediction[Y_test.columns]
    correct = sum(np.logical_and((prediction==1).reshape(Y_train.shape[1]), (Y_test.loc[i]==1).values))
    total = sum(np.logical_or((prediction==1).reshape(Y_train.shape[1]), (Y_test.loc[i]==1).values))
    accuracy = correct/total if total >0 else 1
    acc_iter= np.append(acc_iter,accuracy)
    '''
#Write file
...
Y_predict.to_csv('predictions_resultsC.csv')
